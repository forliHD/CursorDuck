...
export async function onRequestGet({ env }) {
  ...
  const { results } = await env.DB.prepare(
    `SELECT id, title, body, lang, status, votes, version, created FROM ideas
...
export async function onRequestPost({ request, env }) {
  ...
  await env.DB.prepare(
    "INSERT INTO ideas (id, title, body, lang, status, votes, created, voter) VALUES (?1, ?2, ?3, ?4, 'pending', 0, ?5, ?6)"
  ).bind(id, title, text, lang, now, voter).run();
